/**
 * Spotify Enhanced Card  v1.1.5
 *
 * - Uses ha-icon / ha-icon-button for all icons (HA native, theme-aware)
 * - Colours read from sensor.spotify_enhanced_background_color /
 *   sensor.spotify_enhanced_foreground_color (set by Node.js colour service)
 * - CSS verbatim from hui-media-control-card
 * - Background colour bug fixed (image div never overwritten with bg colour)
 */
...
// ─── Icon helpers — use HA native elements ────────────────────────────────────
// ha-icon-button: interactive, 44px hit area, ripple, accessible
// ha-icon: decorative/inline, no interaction
// We size ha-icon-button via --mdc-icon-button-size / --mdc-icon-size CSS vars

const HaBtn = (icon, id = "", title = "", cls = "") =>
  `<ha-icon-button id="${id}" title="${title}" class="ctrl ${cls}" label="${title}">
     <ha-icon icon="${icon}"></ha-icon>
   </ha-icon-button>`;

const HaIco = (icon, size = 20) =>
  `<ha-icon icon="${icon}" style="--mdc-icon-size:${size}px;display:flex;align-items:center"></ha-icon>`;

// ─── MDI icon names (exact variants specified) ────────────────────────────────
const I = {
  play:          "mdi:play",
  pause:         "mdi:pause",
  next:          "mdi:skip-next",
  ...
  shuffle:       "mdi:shuffle",
  shuffle_off:   "mdi:shuffle-disabled",
  repeat:        "mdi:repeat",
  repeat_one:    "mdi:repeat-once",
  repeat_off:    "mdi:repeat-off",
  mic_on:        "mdi:microphone-variant",
  mic_off:       "mdi:microphone-variant-off",
  heart:         "mdi:cards-heart",
  heart_out:     "mdi:cards-heart-outline",
  vol_off:       "mdi:volume-off",
  vol_lo:        "mdi:volume-low",
  vol_hi:        "mdi:volume-high",
  cast:          "mdi:cast",
  search:        "mdi:magnify",
  queue:         "mdi:queue-music",
  library:       "mdi:music-box-multiple",
  close:         "mdi:close",
  dots:          "mdi:dots-vertical",
  add_q:         "mdi:playlist-plus",
  delete:        "mdi:delete",
  chev_r:        "mdi:chevron-right",
  home:          "mdi:home",
  refresh:       "mdi:refresh",
  play_box:      "mdi:play-box-multiple",
};
...
class ProgressTracker {
  constructor() {
    ...
    this._playing = false; this._raf = null; this._drag = false; this._dragPct = 0;
...
class VolDrag {
  constructor(track, fill, thumb, cb) {
    this._t = track; this._f = fill; this._th = thumb; this._cb = cb;
    this._p = 0; this._drag = false;
    if (!track) return;
  ...
  _update(e) {
    ...
    this._p = clamp((e.clientX - r.left) / r.width, 0, 1); this._r();
  }
  _r() {
    const w = `${this._p * 100}%`;
...
class Base extends HTMLElement {
  ...
  // Read colours from the two HA sensors set by the Node.js colour service
  get _bgColor() {
...
// ─── Colour application (fixed) ───────────────────────────────────────────────
// bg → color-block, no-img background, gradient left colour
// fg → player text colour (everything inherits via currentColor)
// image div gets backgroundImage ONLY — never backgroundColor override
...
function applyBg(s, bg, fg, cardH) {
  ...
  // Mirrors HA source imageStyle / gradientStyle exactly:
  // .color-block and .no-img get background-color
  if (block) block.style.backgroundColor = bg || "";
  if (noimg) noimg.style.backgroundColor = bg || "";

  // .image gets background-color AND width (HA sets both in imageStyle)
  // background-color shows before/behind the image loads — it does NOT hide art
  // because background-image takes precedence over background-color in CSS
  if (image) {
    if (bg)    image.style.backgroundColor  = bg;
    if (cardH) image.style.width = `${cardH}px`;
  }

  // .color-gradient: linear-gradient from bg → bg+"00" (transparent), same width as image
  // HA appends "00" to the hex to make transparent: e.g. "#1a2b3c" + "00" = "#1a2b3c00"
  if (grad) {
    grad.style.backgroundImage = bg ? `linear-gradient(to right, ${bg}, ${bg}00)` : "";
    if (cardH) grad.style.width = `${cardH}px`;
  }

  // Foreground colour cascades to all children via CSS currentColor
  if (player) player.style.color = fg || "";
}

// ─── CSS — verbatim from hui-media-control-card + minimal extensions ───────────
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .image {
    ...
    transition: width 0.8s, background-image 0.8s, background-size 0.8s, opacity 0.8s linear 0.8s;
  }
  ...
  /* ha-icon-button sizing — matches HA source ha-icon-button vars */
  ha-icon-button {
  ...
  .vol-track {
    flex: 1; height: 4px; background: rgba(200,200,200,0.5); border-radius: 2px;
    cursor: pointer; position: relative; touch-action: none; transition: height 0.1s;
  }
  ...
  .ph {
    ...
    padding: 8px 8px 6px 16px; flex-shrink: 0;
  ...
  .iph { width: 42px; height: 42px; border-radius: 4px; flex-shrink: 0; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; opacity: 0.6; }
...
<!-- Background — identical to HA source -->
...
<div class="player no-progress" id="player">
  ...
  <div class="top-info">
    <div style="display:flex;align-items:center">
      ${HaBtn(I.search,  "btn-search",  "Search",  "sm")}
      ${HaBtn(I.library, "btn-lib",     "Library", "sm")}
      ${HaBtn(I.cast,    "btn-devices", "Devices", "sm")}
    </div>
    <div style="display:flex;align-items:center">
      ${HaBtn(I.heart_out, "like-btn",   "Save to Liked Songs", "sm")}
      ${HaBtn(I.mic_on,    "btn-lyrics", "Lyrics",              "sm")}
      ${HaBtn(I.queue,     "btn-queue",  "Queue",               "sm")}
      ${HaBtn(I.dots,      "btn-more",   "More info",           "sm")}
    </div>
  </div>
  ...
  <div>
    <div class="title-controls">
      ...
      <div class="controls">
        <div class="start">
          ${HaBtn(I.shuffle_off, "shuf-btn", "Shuffle",    "")}
          ${HaBtn(I.prev,        "prev-btn", "Previous",   "")}
          ${HaBtn(I.play,        "play-btn", "Play/Pause", "pp")}
          ${HaBtn(I.next,        "next-btn", "Next",       "")}
          ${HaBtn(I.repeat_off,  "rep-btn",  "Repeat",     "")}
        </div>
        <div class="end"></div>
      </div>
    </div>
    ...
    <div class="vol-row" id="vol-row">
      ${HaBtn(I.vol_lo, "mute-btn", "Mute", "sm")}
...
<div class="panel" id="panel-search">
  <div class="ph">
    <div class="pt">${HaIco(I.search, 16)}&nbsp;Search</div>
    ${HaBtn(I.close, "cls-search", "Close", "sm")}
  </div>
  <div class="sbar">
    <input class="sinput" id="si" placeholder="Search Spotify…" autocomplete="off"/>
    ${HaBtn(I.search, "sg", "Search", "sm")}
  </div>
...
<div class="panel" id="panel-lib">
  <div class="ph">
    <div class="pt" id="lib-title">${HaIco(I.library, 16)}&nbsp;Library</div>
    ${HaBtn(I.close, "cls-lib", "Close", "sm")}
  </div>
...
<div class="panel" id="panel-devices">
  <div class="ph">
    <div class="pt">${HaIco(I.cast, 16)}&nbsp;Devices</div>
    ${HaBtn(I.close, "cls-devices", "Close", "sm")}
  </div>
...
<div class="panel" id="panel-queue">
  <div class="ph">
    <div class="pt">${HaIco(I.queue, 16)}&nbsp;Queue</div>
    ${HaBtn(I.close, "cls-queue", "Close", "sm")}
  </div>
...
<div class="panel" id="panel-lyrics">
  <div class="ph">
    <div class="pt">${HaIco(I.mic_on, 16)}&nbsp;Lyrics</div>
    ${HaBtn(I.close, "cls-lyrics", "Close", "sm")}
  </div>
  ...
  _bindButtons() {
    ...
    // Hover icon preview (toggle buttons show what clicking will do)
  ...
  _update(prev) {
    ...
    // Progress
    const showP = this._config.show_seek !== false && !this._narrow
      && (this._playing || this._state === "paused") && this._durSecs > 0;
    ...
    s.getElementById("mute-btn")?.querySelector("ha-icon")?.setAttribute("icon",
      this._muted ? I.vol_off : this._vol > 50 ? I.vol_hi : I.vol_lo);
  ...
  _paintShuf() {
    this.shadowRoot.getElementById("shuf-btn")?.querySelector("ha-icon")
      ?.setAttribute("icon", this._shuffle ? I.shuffle : I.shuffle_off);
  }
  _paintRep() {
    this.shadowRoot.getElementById("rep-btn")?.querySelector("ha-icon")
      ?.setAttribute("icon", { off: I.repeat_off, all: I.repeat, one: I.repeat_one }[this._repeat] ?? I.repeat_off);
  }
  _paintLike(liked) {
  ...
  _renderLibRoot() {
    ...
    const roots = [
      ...
      ["spotify://category/liked_songs",     I.heart,    "Liked Songs",     false],
      ["spotify://category/recently_played", I.queue,    "Recently Played", false],
      ["spotify://category/top_tracks",      I.play_box, "Top Tracks",      false],
      ["spotify://category/top_artists",     I.mic_on,   "Top Artists",     true],
      ["spotify://category/new_releases",    I.library,  "New Releases",    true],
      ["spotify://category/featured",        I.play_box, "Featured",        true],
    ];
    body.innerHTML = roots.map(([id, ico, label, exp]) => `
      <div class="item" data-id="${id}" data-exp="${exp}" data-label="${label}">
        <div class="ic">
          <div class="iph">${HaIco(ico, 20)}</div>
  ...
  _bindLib(c) {
    ...
    c.querySelectorAll("[data-aq]").forEach(btn => {
      btn.addEventListener("click", e => { e.stopPropagation(); this._spotify("add_to_queue", { track_uri: btn.dataset.aq }); });
    });
  }
    ...
    try {
      ...
      body.innerHTML = (r.children || []).map(item => {
        ...
        const th = item.thumbnail
          ? `<img class="ithumb${isA ? " circle" : ""}" src="${item.thumbnail}" alt=""/>`
          : `<div class="iph">${HaIco(I.library, 18)}</div>`;
        const rt = !item.can_expand
          ? `<ha-icon-button class="sm" data-aq="${item.media_content_id}" title="Add to queue" label="Add to queue"><ha-icon icon="${I.add_q}"></ha-icon></ha-icon-button>`
          : HaIco(I.chev_r, 18);
  ...
  _updateLibTitle() {
    ...
    if (!el) return;
    if (!this._libStack.length) { el.innerHTML = `${HaIco(I.library, 16)}&nbsp;Library`; return; }
    const crumbs = [
      `<button class="bcc" data-nav="-1">${HaIco(I.home, 13)}&nbsp;Library</button>`,
      ...this._libStack.map((p, i) =>
        `<span class="bcs">›</span><button class="bcc${i===this._libStack.length-1?" last":""}" data-nav="${i}">${p.label}</button>`)
    ].join("");
  ...
  _renderSearch() {
    ...
    const R = this._srR;
    const aqBtn = uri => `<ha-icon-button class="sm" data-aq="${uri}" title="Add to queue" label="Add to queue"><ha-icon icon="${I.add_q}"></ha-icon></ha-icon-button>`;
    ...
    body.innerHTML =
      sec("Tracks", "tracks", mkT, 5) + sec("Albums", "albums", mkA, 4) +
      sec("Artists", "artists", mkAr, 4) + sec("Playlists", "playlists", mkP, 4)
    ...
    body.querySelectorAll("[data-aq]").forEach(btn => btn.addEventListener("click", e => {
      e.stopPropagation(); this._spotify("add_to_queue", { track_uri: btn.dataset.aq });
    }));
    body.querySelectorAll("[data-ex]").forEach(btn => btn.addEventListener("click", () => {
      this._srX[btn.dataset.ex] = true; this._renderSearch();
    }));
  ...
  _renderQueue(body, data) {
    ...
    const mk = (t, now = false) => `
      <div class="item${now ? " now" : ""}" data-uri="${t.uri}">
        ...
        <div class="sdel">${HaIco(I.delete, 20)}</div>
      </div>`;
    let html = "";
  ...
  _renderDevices() {
    ...
    body.innerHTML = devs.map(d => `
      <div class="devitem${d.id === this._devId ? " active" : ""}" data-id="${d.id}">
        <div class="devicon">${HaIco(I.cast, 22)}</div>
    ...
    body.querySelectorAll(".devitem[data-id]").forEach(el =>
      el.addEventListener("click", () => { this._spotify("transfer_playback", { device_id: el.dataset.id }); this._close(); })
    );
  }
  ...
  async _loadLyrics() {
    ...
    try {
      ...
      const r = await fetch(`https://lrclib.net/api/get?${p}`, {
        headers: {
          "Lrclib-Client": "SpotifyEnhancedHA/1.1.5 (https://github.com/Kousei-Uchu/spotify-enhanced)",
          "Accept": "application/json",
        }
      });
      // 404 means track not found (not a network error) — parse JSON to check
      const data = await r.json().catch(() => null);
      if (!data || data.code === "TrackNotFound" || (!data.syncedLyrics && !data.plainLyrics && !data.instrumentalFlag)) {
        throw new Error("not found");
      }
      if (data.instrumentalFlag) {
        body.innerHTML = `<div class="empty">${HaIco(I.mic_on, 28)}<br><br>Instrumental — no lyrics.</div>`;
        return;
      }
      if (data.syncedLyrics) {
        ...
        body.innerHTML = lines.map((l, i) =>
          `<div class="lline${l.words ? "" : " plain"}" data-i="${i}" data-t="${l.ms}">${l.words || "♪"}</div>`
        ).join("");
        body.querySelectorAll(".lline[data-t]").forEach(el =>
        ...
        body.innerHTML = data.plainLyrics.split("\n").map(l =>
          l ? `<div class="lline plain">${l}</div>` : `<div style="height:8px"></div>`
        ).join("");
      } else throw new Error();
    } catch {
      body.innerHTML = `<div class="empty">${HaIco(I.mic_off, 28)}<br><br>Lyrics not available for this track.</div>`;
    }
  }
...
class SpotifyMiniCard extends Base {
  ...
  _build() {
    this.shadowRoot.innerHTML = `<style>${CSS}
      ...
      .vr .vol-track { flex:1; }
      .ctrls { display:flex; align-items:center; flex-shrink:0; }
      ha-icon-button { --mdc-icon-button-size:36px; --mdc-icon-size:22px; color:inherit; }
    ...
    <ha-card>
      ...
      <div class="inf">
        ...
        <div class="vr" id="vr">
          ${HaBtn(I.vol_lo, "mute", "Mute", "")}
          <div class="vol-track" id="vt"><div class="vol-fill" id="vf"></div><div class="vol-thumb" id="vh"></div></div>
        </div>
      </div>
      <div class="ctrls">
        ${HaBtn(I.prev,  "prev", "Previous",   "")}
        ${HaBtn(I.play,  "play", "Play/Pause",  "pp")}
        ${HaBtn(I.next,  "next", "Next",        "")}
      </div>
    </ha-card>`;
    const s = this.shadowRoot;
    ...
    this._vd = new VolDrag(s.getElementById("vt"), s.getElementById("vf"), s.getElementById("vh"),
      p => this._mp("volume_set", { volume_level: round2(p) }));
  }
  ...
  _update() {
    ...
    if (fg) { s.querySelector(".inf").style.color = fg; s.querySelector(".ctrls").style.color = fg; }
  }
}
customElements.define("spotify-mini-card", SpotifyMiniCard);

// ─── Helper to apply bg layer for standalone cards ────────────────────────────

function applyBgStandalone(s, bg, fg, art, hassUrl) {
  // Matches HA imageStyle/gradientStyle exactly
  ...
  if (imgEl) {
    ...
    if (bg) imgEl.style.backgroundColor = bg; // HA sets this too
  }
  const grad = s.getElementById("color-gradient");
...
class SpotifyDeviceCard extends Base {
  ...
  _build() {
    ...
    <ha-card>
      <div class="background no-image off" id="bg"><div class="color-block" id="color-block"></div><div class="no-img" id="no-img"></div><div class="image" id="image"></div><div class="color-gradient" id="color-gradient"></div></div>
      <div class="inner" style="position:relative;z-index:1">
        <div class="hdr">${HaIco(I.cast, 16)}&nbsp;${this._config.title}</div>
  ...
  _update() {
    ...
    list.querySelectorAll(".devitem[data-id]").forEach(el =>
...
class SpotifySearchCard extends Base {
  ...
  _renderResults() {
    ...
    const R = this._r;
    const aqBtn = uri => `<ha-icon-button class="sm" data-aq="${uri}" label="Add to queue"><ha-icon icon="${I.add_q}"></ha-icon></ha-icon-button>`;
    const mkT  = t => `<div class="item" data-play="${t.uri}"><div class="ic"><img class="ithumb" src="${t.album?.images?.[0]?.url||""}" alt=""/><div class="iinfo"><div class="ititle">${t.name}</div><div class="isub">${(t.artists||[]).map(a=>a.name).join(", ")}</div></div>${aqBtn(t.uri)}</div></div>`;
    const mkA  = a => `<div class="item" data-play="${a.uri}"><div class="ic"><img class="ithumb" src="${a.images?.[0]?.url||""}" alt=""/><div class="iinfo"><div class="ititle">${a.name}</div><div class="isub">${(a.artists||[]).map(x=>x.name).join(", ")}</div></div></div></div>`;
    const mkAr = a => `<div class="item" data-play="${a.uri}"><div class="ic"><img class="ithumb circle" src="${a.images?.[0]?.url||""}" alt=""/><div class="iinfo"><div class="ititle">${a.name}</div><div class="isub">Artist</div></div></div></div>`;
    const mkP  = p => `<div class="item" data-play="${p.uri}"><div class="ic"><img class="ithumb" src="${p.images?.[0]?.url||""}" alt=""/><div class="iinfo"><div class="ititle">${p.name}</div><div class="isub">Playlist · ${p.owner?.display_name||""}</div></div></div></div>`;
    ...
    body.querySelectorAll("[data-ex]").forEach(btn => btn.addEventListener("click", () => { this._x[btn.dataset.ex] = true; this._renderResults(); }));
...
class SpotifyQueueCard extends Base {
  ...
  _build() {
    ...
    <ha-card>
      ...
      <div class="inner">
        <div class="hdr">
          <span style="display:flex;align-items:center;gap:6px">${HaIco(I.queue, 16)}&nbsp;Queue</span>
          ${HaBtn(I.refresh, "reload", "Refresh", "sm")}
        </div>
...
class SpotifyLyricsCard extends Base {
  ...
  _build() {
    ...
    <ha-card>
      <div class="background no-image off" id="bg"><div class="color-block" id="color-block"></div><div class="no-img" id="no-img"></div><div class="image" id="image"></div><div class="color-gradient" id="color-gradient"></div></div>
      <div class="inner">
        <div class="hdr">${HaIco(I.mic_on, 16)}&nbsp;Lyrics</div>
        <div class="body" id="body"><div class="loading">Loading…</div></div>
      </div>
    </ha-card>`;
  }
  ...
  async _loadLyrics() {
    ...
    } catch { body.innerHTML = `<div class="empty">${HaIco(I.mic_off, 28)}<br><br>Lyrics not available.</div>`; }
  }
  ...
  _update() {
    applyBgStandalone(this.shadowRoot, this._bgColor, this._fgColor, this._art, u => this._hassUrl(u));
    const so = this._so, tid = this._trackId;
  ...
  _render() {
    ...
    const tog = (k, label, def = true) => `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--divider-color,#eee)">
        <span>${label}</span>
        <ha-switch data-key="${k}" ${c[k] !== false && (c[k] !== undefined ? c[k] : def) ? "checked" : ""}></ha-switch>
      </div>`;
    this.innerHTML = `
...
window.customCards.push(
  { type: "spotify-enhanced-card", name: "Spotify Enhanced — Media Deck",    description: "Full player. Colours via node-vibrant sensor.", preview: true },
