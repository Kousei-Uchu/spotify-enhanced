...
async def async_setup_services(hass: HomeAssistant):
    ...
    svc.async_register(DOMAIN, SERVICE_SAVE_TRACK, _save, vol.Schema({vol.Required(ATTR_TRACK_ID): vol.All(cv.ensure_list, [cv.string])}))
    svc.async_register(DOMAIN, SERVICE_REMOVE_TRACK, _remove, vol.Schema({vol.Required(ATTR_TRACK_ID): vol.All(cv.ensure_list, [cv.string])}))
    ...
    svc.async_register(DOMAIN, SERVICE_FOLLOW_PLAYLIST, _follow, vol.Schema({vol.Required(ATTR_PLAYLIST_ID): cv.string}))
    svc.async_register(DOMAIN, SERVICE_UNFOLLOW_PLAYLIST, _unfollow, vol.Schema({vol.Required(ATTR_PLAYLIST_ID): cv.string}))


async def _play_uri(call: ServiceCall):
    c = _coord(call.hass)
    await c.api.play(device_id=call.data.get(ATTR_DEVICE_ID), context_uri=call.data.get(ATTR_CONTEXT_URI),
                     uris=call.data.get(ATTR_TRACK_URI), position_ms=call.data.get(ATTR_POSITION_MS, 0))
    await c.async_request_refresh()

async def _search(call: ServiceCall):
    c = _coord(call.hass)
    results = await c.api.search(query=call.data[ATTR_QUERY], types=call.data.get(ATTR_SEARCH_TYPE, ["track"]))
    call.hass.bus.async_fire(f"{DOMAIN}_search_results", {"query": call.data[ATTR_QUERY], "results": results})
