...
Every place that says `Kousei-Uchu` needs your actual GitHub username.
...
find . -type f \( -name "*.json" -o -name "*.md" -o -name "*.yml" -o -name "*.sh" \) \
  | xargs sed -i.bak \
      -e "s/Kousei-Uchu/${USERNAME}/g" \
      -e "s|Kousei-Uchu/spotify-enhanced|${USERNAME}/${REPO}|g"
...
Get-ChildItem -Recurse -Include *.json,*.md,*.yml,*.sh | ForEach-Object {
    (Get-Content $_.FullName) `
        -replace 'Kousei-Uchu', $USERNAME `
        -replace 'Kousei-Uchu/spotify-enhanced', "$USERNAME/$REPO" |
